...
- **Fixed** npm package extension not loading under pi: pi's `collectPackageResources`
  silently skipped the package because it lacked a `pi` manifest field. Added
  `pi: { "extensions": ["index.ts"] }` to `pkg.json` so pi detects and loads the
  extension via `loadExtensions` (jiti).
